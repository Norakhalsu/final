...
public class AppointmentController {
    ...
    public ResponseEntity<List<Appointment>> getAppointmentsByDoctor(@AuthenticationPrincipal User user ,@PathVariable String doctorName) {
        List<Appointment> list=appointmentService.findAppointmentByDoctorName(user.getId(), doctorName);
        return ResponseEntity.status(200).body(list);
    }



    public ResponseEntity grt(@AuthenticationPrincipal User patientUser) {
        appointmentService.getAllAppointmentsByUserId(patientUser.getId());
        return ResponseEntity.status(200).body("Appointment Grated successfully");
    }
